document.addEventListener("DOMContentLoaded", function () {
  ...
  function filterRows() {
    ...
    const rows = pendingTable.querySelectorAll(".teacher-row");
  ...
  document.getElementById("importCsv").addEventListener("click", () => {
    // Check if user is admin - only admins can import teacher attendance
    const userRole = document.querySelector("body").dataset.userRole || "Admin";
    if (userRole === "Admin") {
      showImportModal();
    } else {
      alert("Access denied. Only admins can import teacher attendance data.");
    }
  });
  ...
  document.getElementById("exportCsv").addEventListener("click", () => {
    // Check if user is admin - if yes, export current date data directly
    const userRole = document.querySelector("body").dataset.userRole || "Admin";
    if (userRole === "Admin") {
      handleAdminCsvExport();
    } else {
      handleCsvExport();
    }
  });
  ...
  function showImportModal() {
    ...
    modalContent.innerHTML = `
      <h3>Import Teacher Attendance from CSV</h3>

      <div style="margin: 20px 0;">
        <h4>CSV Format Requirements:</h4>
        <ul style="margin: 10px 0; padding-left: 20px;">
          <li>Required columns: <code>teacher_name</code>, <code>subject</code>, <code>status</code>, <code>date</code></li>
        ...
        <div style="margin-top: 15px;">
          <a href="/attendance/teacher-template/" class="btn btn-outline" style="font-size: 14px; padding: 8px 12px;">
  ...
  function showExcelImportModal() {
    ...
    modalContent.innerHTML = `
      <h3>Import Teacher Attendance from Excel</h3>
      ...
      <div style="margin: 20px 0;">
        ...
        <div style="margin-top: 15px;">
          <a href="/attendance/teacher-excel-template/" class="btn btn-outline" style="font-size: 14px; padding: 8px 12px;">
  ...
  function handleCsvImport(file, modal, importBtn) {
    ...
    fetch("/attendance/import-teacher-csv/", {
      method: "POST",
      body: formData,
    })
      ...
      .then((data) => {
        ...
        if (data.success) {
          ...
          successMessage.textContent = `Successfully imported ${data.imported_count} teacher attendance records. Data is now in pending state - please use "Commit Attendance" to finalize.`;
  ...
  function handleExcelImport(file, modal, importBtn) {
    ...
    fetch("/attendance/import-teacher-excel/", {
      method: "POST",
      body: formData,
    })
      .then((response) => response.json())
  ...
  // Admin CSV Export Handler (direct export for current date)
  function handleAdminCsvExport() {
    const today = new Date().toISOString().split("T")[0]; // Get current date in YYYY-MM-DD format
    const url = `/attendance/export-teacher-csv/?from_date=${today}`;
    window.open(url, "_blank");
  }

  // CSV Export Handler (modal for other roles if any)
  function handleCsvExport() {
    ...
    exportBtn.addEventListener("click", () => {
      ...
      let url = "/attendance/export-teacher-csv/?";
      const params = [];
  ...
  // Admin Excel Export Handler (direct export for current date)
  function handleAdminExcelExport() {
    const today = new Date().toISOString().split("T")[0]; // Get current date in YYYY-MM-DD format
    const url = `/attendance/export-teacher-excel/?from_date=${today}`;
    window.open(url, "_blank");
  }

  // Excel Export Handler (modal for other roles if any)
  function handleExcelExport() {
    ...
    modalContent.innerHTML = `
      <h3>Export Teacher Attendance Data to Excel</h3>
      <div style="margin: 15px 0;">
        <label for="fromDate">Export From Date:</label><br>
        <input type="date" id="fromDate" style="width: 100%; padding: 8px; margin: 5px 0;">
        <small style="color: #666;">Leave empty to export all teacher attendance data</small>
    ...
    exportBtn.addEventListener("click", () => {
      ...
      // Build URL with parameters
      let url = "/attendance/export-teacher-excel/?";
      const params = [];
      if (fromDate) params.push(`from_date=${fromDate}`);
      url += params.join("&");
  ...
  // Admin JSON Export Handler (direct export for current date)
  function handleAdminJsonExport() {
    const today = new Date().toISOString().split("T")[0]; // Get current date in YYYY-MM-DD format
    const url = `/attendance/export-teacher-json/?from_date=${today}`;
    window.open(url, "_blank");
  }

  // JSON Export Handler (modal for other roles if any)
  function handleJsonExport() {
    ...
    modalContent.innerHTML = `
      <h3>Export Teacher Attendance Data to JSON</h3>
      <div style="margin: 15px 0;">
        <label for="fromDate">Export From Date:</label><br>
        <input type="date" id="fromDate" style="width: 100%; padding: 8px; margin: 5px 0;">
        <small style="color: #666;">Leave empty to export all teacher attendance data</small>
    ...
    exportBtn.addEventListener("click", () => {
      ...
      // Build URL with parameters
      let url = "/attendance/export-teacher-json/?";
      const params = [];
      if (fromDate) params.push(`from_date=${fromDate}`);
      url += params.join("&");
